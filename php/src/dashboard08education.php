...
$sql2 = "SELECT
  SUM(CASE WHEN hn.HOS_TYPE in ('กรมสุขภาพจิต')AND p.positiontypeID = '8' THEN 1 ELSE 0 END) AS 'MA01',
  SUM(CASE WHEN hn.HOS_TYPE in ('โรงพยาบาลศูนย์','โรงพยาบาลทั่วไป')AND p.positiontypeID = '8' THEN 1 ELSE 0 END) AS 'MA02',
  SUM(CASE WHEN hn.HOS_TYPE in ('โรงพยาบาลชุมชน') AND p.positiontypeID = '8' THEN 1 ELSE 0 END) AS 'MA03',
...
if (isset($_POST['Year'])) {
  ...
  if (isset($_POST['CODE_PROVINCE'])) {
	if ($_POST['CODE_PROVINCE']<> 'ทั้งหมด') {
	$CODE_PROVINCE = $_POST['CODE_PROVINCE'];
	$MOOsql1 = $MOOsql1."AND p.HospitalID = '".$CODE_PROVINCE."'" ;
	}
  }
  $MOOsql1 = $MOOsql1."
...
if (isset($_POST['Year'])) {
  ...
  if (isset($_POST['CODE_PROVINCE'])) {
	if ($_POST['CODE_PROVINCE']<> 'ทั้งหมด') {
	$CODE_PROVINCE = $_POST['CODE_PROVINCE'];
	$MOOsql1p = $MOOsql1p."AND p.HospitalID = '".$CODE_PROVINCE."'" ;
	}
  }
  $MOOsql1p = $MOOsql1p."
