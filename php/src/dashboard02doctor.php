...
if (isset($_POST['TYPE_SERVICE'])) {
  ...
  $sql1 = $sql1."AND e.HOS_TYPE = '".$mySelect."'" ;
  }
}
...
if (isset($_POST['TYPE_SERVICE'])) {
  if (trim($_POST['TYPE_SERVICE'])<> 'ทั้งหมด') {
  $mySelect = trim($_POST['TYPE_SERVICE']);
  $msql1 = $msql1."AND h.HOS_TYPE = '".$mySelect."'" ;
  }
}
...
if (isset($_POST['Year'])) {
	$Year = $_POST['Year']-543;
	$msql2 = $msql2."AND p.personnelDate >= CONCAT(".$Year-1 .",'-10-01') 
	AND p.personnelDate <= CONCAT(".$Year.", '-09-30')";
  }else{
	if (date("m") == '10' || date("m") == '11' || date("m") == '12'){
		$Year = (date("Y"))+1;
...
<script>
        ...
        document.getElementById('resetButton').addEventListener('click', function() {
           ...
           window.location.href = 'dashboard02doctor.php'; 
        });
