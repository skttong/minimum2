...
/*
$sqlall = "SELECT b.HospitalID, e.HOS_NAME,
count(SUBSTRING_INDEX(b.training, ',', 1)) AS nu01,
count(SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 2), ',', -1)) AS nu02,
count(SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 3), ',', -1)) AS nu03,
count( SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 4), ',', -1)) AS nu04,
count(SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 5), ',', -1)) AS nu05,
count( SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 6), ',', -1)) AS nu06,
count(SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 7), ',', -1)) AS nu07
FROM personnel b
JOIN hospitalnew e ON e.CODE5 = b.HospitalID
WHERE b.positiontypeID = '2' AND b.setdel = '1'
  ";
...
if (isset($_POST['Year'])) {
  $Year = $_POST['Year']-543;
  $sqlall = $sqlall."AND YEAR(b.personnelDate) = '".$Year."'" ;
}

if (isset($_POST['CODE_HMOO'])) {
  if ($_POST['CODE_HMOO']<> 'ทั้งหมด') {
  $CODE_HMOO = $_POST['CODE_HMOO'];
  $sqlall = $sqlall."AND e.CODE_HMOO = '".$CODE_HMOO."'" ;
  }
}
...
if (isset($_POST['TYPE_SERVICE'])) {
  ...
  $sqlall = $sqlall."AND e.TYPE_SERVICE = '".$mySelect."'" ;
  }
}
...
if (isset($_POST['CODE_PROVINCE'])) {
	if ($_POST['CODE_PROVINCE']<> 'ทั้งหมด') {
	$CODE_PROVINCE = $_POST['CODE_PROVINCE'];
	$sqlall = $sqlall."AND e.NO_PROVINCE = '".$CODE_PROVINCE."'" ;
	}
  }
  ...
  if (isset($_POST['CODE_HOS'])) {
	if ($_POST['CODE_HOS']<> 'ทั้งหมด') {
	$CODE_HOS = $_POST['CODE_HOS'];
	$sqlall = $sqlall."AND b.HospitalID = '".$CODE_HOS."'" ;
	}
  }

$sqlall=$sqlall."
GROUP BY b.HospitalID,e.HOS_NAME;"
  ;
*/
...
$sqlall = "SELECT 
    hosn.HOS_NAME, 
    b.HospitalID, 
    COUNT(CASE WHEN SUBSTRING_INDEX(b.training, ',', 1) = 'ยังไม่ผ่านการอบรมเฉพาะทาง' THEN 1 END) AS 'nu01', 
    COUNT(CASE WHEN SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 2), ',', -1) = 'การพยาบาลเฉพาะทางจิตเวชสุขภาพจิตและจิตเวช (จิตเวชผู้ใหญ่)' THEN 1 END) AS 'nu02', 
    COUNT(CASE WHEN SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 3), ',', -1) = 'การพยาบาลเฉพาะสุขภาพจิตและจิตเวชผู้สูงอายุ' THEN 1 END) AS 'nu03', 
    COUNT(CASE WHEN SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 4), ',', -1) = 'การพยาบาลเฉพาะทางจิตเวชเด็กและวัยรุ่น' THEN 1 END) AS 'nu04', 
    COUNT(CASE WHEN SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 5), ',', -1) = 'การพยาบาลเฉพาะทางผู้ใช้ยาและสารเสพติด' THEN 1 END) AS 'nu05', 
    COUNT(CASE WHEN SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 6), ',', -1) = 'การพยาบาลเฉพาะทางผู้ใช้ยาและสารเสพติด ระยะสั้น 10 วัน' THEN 1 END) AS 'nu06', 
    COUNT(CASE WHEN SUBSTRING_INDEX(SUBSTRING_INDEX(b.training, ',', 7), ',', -1) = 'อื่น ๆ' THEN 1 END) AS 'nu07'
FROM 
    hospitalnew hosn
JOIN 
    personnel b ON hosn.CODE5 = b.HospitalID
WHERE 
    b.positiontypeID = '2' 
    AND b.setdel = '1' ";
...
if (isset($_POST['Year'])) {
	$Year = $_POST['Year']-543;
	$sqlall = $sqlall."AND b.personnelDate >= CONCAT(".$Year-1 .",'-10-01') 
	AND b.personnelDate <= CONCAT(".$Year.", '-09-30')";
  }else{
	if (date("m") == '10' || date("m") == '11' || date("m") == '12'){
		$Year = (date("Y"))+1;
...
if (isset($_POST['CODE_HMOO'])) {
  if ($_POST['CODE_HMOO']<> 'ทั้งหมด') {
  $CODE_HMOO = $_POST['CODE_HMOO'];
  $sqlall = $sqlall."AND hosn.CODE_HMOO = '".$CODE_HMOO."'" ;
  }
}
...
if (isset($_POST['type_Affiliation'])) {
  if (trim($_POST['type_Affiliation'])<> 'ทั้งหมด') {
  $type_Affiliation = trim($_POST['type_Affiliation']);
  $sqlall = $sqlall."AND hosn.type_Affiliation LIKE ('".$type_Affiliation."%')" ;
  }
}
...
if (isset($_POST['Affiliation'])) {
  if (trim($_POST['Affiliation'])<> 'ทั้งหมด') {
  $Affiliation = trim($_POST['Affiliation']);
  $sqlall = $sqlall."AND hosn.Affiliation = '".$Affiliation."'" ;
  }
}
...
if (isset($_POST['TYPE_SERVICE'])) {
  if (trim($_POST['TYPE_SERVICE'])<> 'ทั้งหมด') {
  $mySelect = trim($_POST['TYPE_SERVICE']);
  $sqlall = $sqlall."AND hosn.HOS_TYPE = '".$mySelect."'" ;
  }
}
...
if (isset($_POST['CODE_PROVINCE'])) {
	if ($_POST['CODE_PROVINCE']<> 'ทั้งหมด') {
	$CODE_PROVINCE = $_POST['CODE_PROVINCE'];
	$sqlall = $sqlall."AND hosn.NO_PROVINCE = '".$CODE_PROVINCE."'" ;
	}
  }
   ...
   <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
		  		<h3>ข้อมูลทรัพยากรบุคลากร พยาบาล </h3>
          </div>
          <div class="col-sm-6">
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="card card-default">
          ...
          <!-- /.card-header -->
          <div class="card-body">
			<form class="form-valide" action="dashboard02nurse.php" method="post" id="myform1" name="foml">  
    ...
    if (selectedValue) {
      ...
      <div class="row">
        ...
        const myChart3 = new Chart(ctx, {
            type: 'bar',
			data: {
        labels: [ 'พยาบาลเฉพาะทางสุขภาพจิตและจิตเวช', 'พยาบาลเฉพาะทางจิตเวชเด็กและวัยรุ่น', 'พยาบาลเฉพาะทางผู้ใช้ยาและสารเสพติด', 'พยาบาลเฉพาะทางผู้สูงอายุ','ยังไม่ผ่านการอบรมเฉพาะทาง'],
        datasets: [{
            label: 'ปฏิบัติงาน',
    ...
    link.click();
			...
			<div class="card">
				...
				<table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr align="center">
					  ...
					  <th width="15%">พยาบาล PG สุขภาพจิต</th>
					  <th width="15%">พยาบาล PG จิตเวชเด็ก</th>
				   </tr>
                   </thead>
                  <tbody>
				  <?php
				  ...
				  <tr align="center">
						...
						<td width="12%"><?php echo $rowall2['nu02'];?></td>
						<td width="12%"><?php echo $rowall2['nu04'];?></td>
		...
		<div class="row">
			<div class="col-lg-6">
				...
				<p><?php echo number_format((($nu02 / $Total)*100000), 4, '.', ',');?>  : 1แสน ประชากร</p>
			  ...
			  <div class="col-lg-6">
                ...
                <div class="inner">
                    ...
                    <p><?php echo number_format((($nu04 / $Total)*100000), 4, '.', ',');?>  : 1แสน ประชากร</p>
...
<script>
        ...
        document.getElementById('resetButton').addEventListener('click', function() {
           ...
           window.location.href = 'dashboard02nurse.php'; 
        });
